...
public class VerticalController extends View {
    ...
    public VerticalController(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            final TypedArray a = context.obtainStyledAttributes(attrs,
                    R.styleable.VerticalController, 0, 0);

            mPoints = a.getInteger(R.styleable.VerticalController_points, mPoints);
            mMax = a.getInteger(R.styleable.VerticalController_max, mMax);
            mMax = a.getInteger(R.styleable.VerticalController_max, mMax);
            mStep = a.getInteger(R.styleable.VerticalController_step, mStep);
            mDefaultValue = a.getInteger(R.styleable.VerticalController_defaultValue, mDefaultValue);
            mCornerRadius = a.getInteger(R.styleable.VerticalController_cornerRadius, mCornerRadius);
            mtextBottomPadding = a.getInteger(R.styleable.VerticalController_textBottomPadding, mtextBottomPadding);
            //Images
            mImageEnabled = a.getBoolean(R.styleable.VerticalController_imageEnabled, mImageEnabled);

            if (mImageEnabled){
                Assert.assertNotNull("When images are enabled, defaultImage can not be null. Please assign a drawable in the layout XML file", a.getDrawable(R.styleable.VerticalController_defaultImage));
                Assert.assertNotNull("When images are enabled, minImage can not be null. Please assign a drawable in the layout XML file", a.getDrawable(R.styleable.VerticalController_minImage));
                Assert.assertNotNull("When images are enabled, maxImage can not be null. Please assign a drawable in the layout XML file", a.getDrawable(R.styleable.VerticalController_maxImage));

                mDefaultImage = ((Drawable) a.getDrawable(R.styleable.VerticalController_defaultImage));
                mMinImage = ((Drawable) a.getDrawable(R.styleable.VerticalController_minImage));
                mMaxImage = ((Drawable) a.getDrawable(R.styleable.VerticalController_maxImage));
            }

            progressColor = a.getColor(R.styleable.VerticalController_progressColor, progressColor);
            backgroundColor = a.getColor(R.styleable.VerticalController_backgroundColor, backgroundColor);

            mTextSize = (int) a.getDimension(R.styleable.VerticalController_textSize, mTextSize);
            textColor = a.getColor(R.styleable.VerticalController_textColor, textColor);
            text = a.getString(R.styleable.VerticalController_text);

            mEnabled = a.getBoolean(R.styleable.VerticalController_enabled, mEnabled);
            mTouchDisabled = a.getBoolean(R.styleable.VerticalController_touchDisabled, mTouchDisabled);
            mtextEnabled = a.getBoolean(R.styleable.VerticalController_textEnabled, mtextEnabled);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mImageEnabled && mDefaultImage != null && mMinImage != null && mMaxImage != null){
            ...
            if (mtextEnabled){
                String strPoint=text;
                if(TextUtils.isEmpty(text))
                    strPoint = String.valueOf(mPoints);
    ...
    private void drawIcon(Drawable drawable, Canvas canvas){
        Bitmap bitmap = getResizedBitmap(drawableToBitmap(drawable),canvas.getWidth()/2, canvas.getWidth()/2);
    ...
    public interface OnValuesChangeListener {
         ...
         */
        void onPointsChanged(VerticalController boxedPoints, int points);
        void onStartTrackingTouch(VerticalController boxedPoints);
        void onStopTrackingTouch(VerticalController boxedPoints);
    }
